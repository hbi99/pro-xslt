...
describe("importStylesheet reset", () => {
    it("makes new templates available immediately after re-import", () => {
        let { xmlDoc, xslDoc: xsl1 } = loadXml(`importStylesheet-reset/reset-pass-one.xml`);
        let p = new ProXslt();
        ...
        p.importStylesheet(loadXml(`importStylesheet-reset/reset-pass-two.xml`).xslDoc);
        ...
        p.importStylesheet(loadXml(`importStylesheet-reset/reset-pass-three.xml`).xslDoc);
